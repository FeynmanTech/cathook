...
void Misc::PaintTraverse(void*, unsigned int, bool, bool) {
	...
	/*if (!interfaces::input->IsButtonDown(ButtonCode_t::KEY_F)) {
		...
		if (g_pLocalPlayer->weapon) {
			...
			AddSideString(colors::white, colors::black, "flNextPrimaryAttack: %f", GetEntityValue<float>(g_pLocalPlayer->weapon, netvar.flNextPrimaryAttack));
			AddSideString(colors::white, colors::black, "nTickBase: %f", (float)(GetEntityValue<int>(g_pLocalPlayer->entity, netvar.nTickBase)) * interfaces::gvars->interval_per_tick);
			AddSideString(colors::white, colors::black, "CanShoot: %i", BulletTime());
			AddSideString(colors::white, colors::black, "Decaps: %i", GetEntityValue<int>(g_pLocalPlayer->entity, netvar.iDecapitations));
			AddSideString(colors::white, colors::black, "Damage: %f", GetEntityValue<float>(g_pLocalPlayer->weapon, netvar.flChargedDamage));
			AddSideString(colors::white, colors::black, "DefIndex: %i", GetEntityValue<int>(g_pLocalPlayer->weapon, netvar.iItemDefinitionIndex));
			...
			AddSideString(colors::white, colors::black, "ambassador_can_headshot: %i", (interfaces::gvars->curtime - GetEntityValue<float>(g_pLocalPlayer->weapon, netvar.flLastFireTime)) > 0.95);
